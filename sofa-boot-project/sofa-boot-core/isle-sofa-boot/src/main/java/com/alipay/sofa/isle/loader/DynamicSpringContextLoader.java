...
public class DynamicSpringContextLoader implements SpringContextLoader {
        ...
        if (deployment.getSpringParent() != null) {
            ...
            if (StringUtils.hasText(springParent)) {
                ...
                if (parent != null) {
                    ...
                    if (parentSpringContext == null) {
                        SofaLogger.warn("Module [{}]'s Spring-Parent [{}] is Null!", deployment.getModuleName(), springParent);
                    }
                }
            }
        }
    ...
    private void addPostProcessors(DefaultListableBeanFactory beanFactory) {
        Map<String, BeanDefinition> processors = (Map<String, BeanDefinition>) rootApplicationContext.getBean(SofaBootConstants.PROCESSORS_OF_ROOT_APPLICATION_CONTEXT);
