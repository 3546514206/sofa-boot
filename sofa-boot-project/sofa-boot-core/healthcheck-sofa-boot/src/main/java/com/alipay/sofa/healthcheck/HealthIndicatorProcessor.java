...
public class HealthIndicatorProcessor {
    ...
    private static Logger logger = HealthCheckLoggerFactory.getLogger(HealthIndicatorProcessor.class);
    ...
    public boolean readinessHealthCheck(Map<String, Health> healthMap) {
        ...
        boolean result = healthIndicators.entrySet().stream().filter(entry -> !(entry.getValue() instanceof NonReadinessCheck)).map(entry -> doHealthCheck(entry.getKey(), entry.getValue(), healthMap)).reduce(true, BinaryOperators.andBoolean());
        if (result) {
