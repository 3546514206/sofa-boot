...
public class HealthCheckerProcessor {
    ...
    private static Logger logger = HealthCheckLoggerFactory.getLogger(HealthCheckerProcessor.class);
    ...
    public boolean livenessHealthCheck(Map<String, Health> healthMap) {
        ...
        boolean result = healthCheckers.entrySet().stream().map(entry -> doHealthCheck(entry.getKey(), entry.getValue(), false, healthMap, false)).reduce(true, BinaryOperators.andBoolean());
        if (result) {
    ...
    public boolean readinessHealthCheck(Map<String, Health> healthMap) {
        ...
        boolean result = healthCheckers.entrySet().stream().filter(entry -> !(entry.getValue() instanceof NonReadinessCheck)).map(entry -> doHealthCheck(entry.getKey(), entry.getValue(), true, healthMap, true)).reduce(true, BinaryOperators.andBoolean());
        if (result) {
    ...
    private boolean doHealthCheck(String beanId, HealthChecker healthChecker, boolean isRetry, Map<String, Health> healthMap, boolean isReadiness) {
        ...
        do {
            ...
            if (isRetry && retryCount < healthChecker.getRetryCount()) {
                try {
                    ...
                    logger.error(String.format("Exception occurred while sleeping of %d retry HealthChecker[%s] %s check.", retryCount, beanId, checkType), e);
                }
            }
        ...
        try {
            if (!result) {
                logger.error("HealthChecker[{}] {} check fail with {} retry; fail details:{}; strict mode:{}", beanId, checkType, retryCount, objectMapper.writeValueAsString(health.getDetails()), healthChecker.isStrictCheck());
            }
        } catch (JsonProcessingException ex) {
            logger.error(String.format("Error occurred while doing HealthChecker %s check.", checkType), ex);
        }
